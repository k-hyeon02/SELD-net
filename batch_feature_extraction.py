...
for ovo in [1, 2, 3]:  # Sound Event overlap : overlap(동시에 겹치는 음원 수) 1,2,3 순회, Change to [1] if you are only calculating the features for overlap 1.
    for splito in [1, 2, 3]:    # all splits. Use [1, 8, 9] for 'real' and 'mreal' datasets. Change to [1] if you are only calculating features for split 1.
        for nffto in [512]:  # For now use 512 point FFT. Once you get the code running, you can play around with this.
            ...
            feat_cls.extract_all_feature()  # wav → 스펙트로그램 → .npy 저장
            feat_cls.preprocess_features()  # train으로 scaler fit → 전체 파일 정규화 → .npy 저장

            # # Extract labels in regression mode
            feat_cls.extract_all_labels('regr', 0)  # csv → SE/DOA 레이블 배열 → .npy 저장